...
static void get_max_brightness(void) {
    bus_call(&br.max, "i", "getmaxbrightness", "s", conf.screen_path);
}

static void get_current_brightness(void) {
    bus_call(&br.current, "i", "getbrightness", "s", conf.screen_path);
}

double set_brightness(double perc) {
    bus_call(&br.current, "i", "setbrightness", "si", conf.screen_path, (int) (br.max * perc));
    if (br.current != -1) {
...
double capture_frame(void) {
    ...
    bus_call(&brightness, "d", "captureframe", "s", conf.dev_name);
    return brightness;
}
