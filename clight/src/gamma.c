...
void check_gamma_time(const float lat, const float lon, struct time *t) {
    ...
    } else if (ret_sunrise == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
    } else {
        printf("No sunrise today...pretty boring, isn't it?\n");
    }
    ret_sunset = calculate_sunset(lat, lon, &sunset, 0);
    if (ret_sunset == 0) {
        printf("Sunset: %s", ctime(&sunset));
    } else if (ret_sunset == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
    } else {
        printf("No sunset today, enjoy your long day!\n");
    }
    
    // if sunrise time (30mins before) is after now, set next alarm
    if (ret_sunrise == 0 && sunrise - 30 * 60 > now) {
        t->state = NIGHT;
        t->next_alarm = sunrise - 30 * 60;
    } else if (ret_sunset == 0 && sunset - 30 * 60 > now) { // alarm 30mins before
        t->state = DAY;
        t->next_alarm = sunset - 30 * 60;
    } else {
        calculate_sunrise(lat, lon, &sunrise, 1);
        t->state = NIGHT;
        t->next_alarm = sunrise - 30 * 60; // alarm 30mins before
    }
}
