...
static void init(void) {
    int r = idle_init(client, &slot, conf.dim_conf.dimmer_timeout[state.ac_state], on_new_idle);
    if (r == 0) {
        M_SUB(UPOWER_UPD);
        M_SUB(INHIBIT_UPD);
        M_SUB(DIMMER_TO_REQ);
...
static bool evaluate(void) {
    return !conf.dim_conf.no_dimmer && state.ac_state != -1;
}
...
static void receive(const msg_t *const msg, UNUSED const void* userdata) {
    ...
    case DIMMER_TO_REQ: {
        ...
        if (VALIDATE_REQ(up)) {
            conf.dim_conf.dimmer_timeout[up->state] = up->new;
    ...
    case SIMULATE_REQ: {
        ...
        if (VALIDATE_REQ((void *)msg->ps_msg->message)) {
            idle_client_reset(client, conf.dim_conf.dimmer_timeout[state.ac_state]);
        }
        break;
    }
    default:
...
static void upower_timeout_callback(void) {
    idle_set_timeout(client, conf.dim_conf.dimmer_timeout[state.ac_state]);
}
...
static void inhibit_callback(void) {
    if (!state.inhibited) {
        DEBUG("Being resumed.\n");
        idle_client_start(client, conf.dim_conf.dimmer_timeout[state.ac_state]);
        m_unbecome();
    } else {
        DEBUG("Being paused.\n");
